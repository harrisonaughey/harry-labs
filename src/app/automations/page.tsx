...
export default async function AutomationsPage() {
  ...
  return (
    ...
    >
      {/* Active flows */}
      <div className="rounded-xl overflow-hidden mb-6" style={{ background: "#111118", border: "1px solid #1e1e2e" }}>
        <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid #1e1e2e" }}>
          <div>
            <h2 className="text-sm font-semibold text-white">Klaviyo Flows</h2>
            <p className="text-xs mt-0.5" style={{ color: "#6b7280" }}>Automated email sequences — synced from Klaviyo</p>
          </div>
          <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#1e1e2e", color: "#6b7280" }}>
            {flows.length} flows
          </span>
        </div>
        {flows.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-sm mb-2" style={{ color: "#4b5563" }}>No flows synced yet</p>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {flows.map((f: any) => {
                const s = STATUS_STYLE[f.status?.toLowerCase()] ?? { bg: "#1e1e2e", text: "#9ca3af" };
                return (
                  <tr key={f.flow_id} className="hover:bg-white/[0.02]" style={{ borderTop: "1px solid #1a1a24" }}>
                    <td className="px-4 py-3 text-white font-medium">{f.flow_name}</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded-full text-xs capitalize"
                        style={{ background: s.bg, color: s.text }}>{f.status}</span>
                    </td>
                    <td className="px-4 py-3" style={{ color: "#9ca3af" }}>{(f.recipients ?? 0).toLocaleString()}</td>
                    <td className="px-4 py-3" style={{ color: "#9ca3af" }}>{((f.open_rate ?? 0) * 100).toFixed(1)}%</td>
                    <td className="px-4 py-3" style={{ color: "#9ca3af" }}>{((f.click_rate ?? 0) * 100).toFixed(1)}%</td>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="rounded-xl p-6" style={{ background: "#111118", border: "1px solid #6366f130" }}>
          ...
          <div className="space-y-2">
            {["Order → Tag customer → Send email → Delay 3d → Follow-up",
              ...
              <div key={ex} className="text-xs px-3 py-2 rounded-lg" style={{ background: "#0d0d14", color: "#4b5563" }}>
                {ex}
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl p-6" style={{ background: "#111118", border: "1px solid #1e1e2e" }}>
          <div className="flex items-center gap-3 mb-3">
            <span className="text-2xl">✦</span>
            <div>
              <p className="text-sm font-semibold text-white">AI Flow Optimisation</p>
